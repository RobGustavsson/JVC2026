...
def parse_match_row(tr: Tag) -> dict | None:
    ...
    # tds[5] holds two team divs and a center span (either "-" or score)
    match_cell = tds[5]
    ...
    center = match_cell.find("span", style=re.compile("font-weight:700"))
    center_text = text(center) if center else "-"
    score_match = SCORE_RE.match(center_text.replace("–", "-"))
    resultat = f"{score_match.group(1)}-{score_match.group(2)}" if score_match else None
...
def parse_groups_for_class(html: str, klass: str) -> dict:
    ...
    then fetch each such group's standings table."""
    soup = BeautifulSoup(html, "lxml")
    ...
    for card in cards:
        ...
        if not any(involves_hk(t) for t in team_names):
            continue
        # Fetch the standings page for this group
        try:
            time.sleep(2)
